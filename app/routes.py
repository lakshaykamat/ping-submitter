...
def register_routes(app):
    ...
    def dashboard():
        ...
        return render_template("dashboard.html", jobs=jobs, sites=sites, events=events)
    ...
    def run_job_from_page(job_id):
        ...
        thread = Thread(target=run_submission_job, kwargs={"job_id": job_id, "app": app}, daemon=True)
        thread.start()
    ...
    @app.get("/captcha/<int:challenge_id>")
    def captcha_challenge(challenge_id):
        challenge = get_captcha_challenge(challenge_id)
        if challenge is None:
            return render_template("captcha.html", challenge=None), 404
        return render_template("captcha.html", challenge=challenge)

    @app.post("/captcha/<int:challenge_id>")
    def answer_captcha(challenge_id):
        try:
            challenge = save_captcha_answer(challenge_id, request.form.get("answer"))
        except ValidationError as error:
            return jsonify({"error": str(error)}), 400
        if challenge is None:
            return render_template("captcha.html", challenge=None), 404
        return redirect(url_for("job_detail", job_id=challenge.job_id))

    @app.get("/captcha/<int:challenge_id>/screenshot")
    def captcha_screenshot(challenge_id):
        challenge = get_captcha_challenge(challenge_id)
        if challenge is None or not challenge.screenshot_path:
            return jsonify({"error": "captcha screenshot not found"}), 404
        return send_file(challenge.screenshot_path)
